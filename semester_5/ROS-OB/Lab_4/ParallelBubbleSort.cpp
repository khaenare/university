 ...
 * Головний файл паралельної версії (Odd-Even Transposition)
 */
...
// --- Forward declarations (оголошення функцій з Test файлу) ---
...
// Проста ініціалізація (зворотний порядок)
...
// Ініціалізація пам'яті та змінних
void ProcessInitialization(double *&pData, int& DataSize, double *&pProcData, int& BlockSize) {
    ...
    for(int i = 0; i < ProcRank; i++)
        RestData = RestData - RestData / (ProcNum - i);
...
// Основна функція паралельного сортування
void ParallelBubble(double *pProcData, int BlockSize) {
    ...
    for(int i = 0; i < ProcNum; i++) {
        if((i % 2) == 1) {
            if((ProcRank % 2) == 1) {
                Offset = 1; SplitMode = KeepFirstHalf;
            } else {
                Offset = -1; SplitMode = KeepSecondHalf;
            }
        } else {
            if((ProcRank % 2) == 1) {
                Offset = -1; SplitMode = KeepSecondHalf;
...
// Функція для тестування коректності результату
...
// Функція для налагоджувального виведення
...
int main(int argc, char *argv[]) {
    ...
    // Створення копії для перевірки (тільки на Root процесі)
    if (ProcRank == 0) {
    ...
    // Тестування (можна закоментувати при вимірах часу)
